...
为了容错，每个分区可以有多个**副本**，分散在不同的 Broker 上。同一分区的副本中：

- **Leader 副本**：负责处理该分区的所有读写请求
- **Follower 副本**：主动从 Leader 拉取数据，保持同步备份

**ISR（In-Sync Replicas，同步副本集合）** 是指与 Leader 数据同步程度满足要求的副本集合。只有 ISR 中的副本才有资格在 Leader 宕机时被 Controller 选为新的 Leader，从而保证数据不丢失。
